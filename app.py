...
map_data['Active_24hr'] = map_data['Confirmed_24hr'] - (map_data['Deaths_24hr'] + map_data['Recovered_24hr'])
    ...
    if map_disp_type=="confirmed":
        return {
            "data": [{
                ...
                # "hoverinfo": "text",         
                "hovertext": [["Country/Region: {} <br>Province/State: {} <br>Active: {} (+ {} past 24hrs)<br>Confirmed: {} (+ {} past 24hrs)<br>Deaths: {} (+ {} past 24hrs)<br>Recovered: {} (+ {} past 24hrs)".format(i, j, k, k24, l, l24, m, m24, n, n24)]
                              for i, j, k, l, m, n, k24, l24, m24, n24 in zip(map_data['Country/Region'], map_data['Province/State'],
                                                                              map_data['Active'], map_data['Confirmed'], map_data['Deaths'], map_data['Recovered'],
                                                                              map_data['Active_24hr'],map_data['Confirmed_24hr'], map_data['Deaths_24hr'], map_data['Recovered_24hr'],)],
...
def draw_singleCountry_Scatter(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,selected_row=0, daily_change=False):
    ...
    fig.add_trace(go.Scatter(x=df_confirmed_t.index, y=df_confirmed_t.iloc[:,selected_row],
                             ...
                             line=dict(color='#3372FF', width=2),
                             fill='tozeroy',))
    fig.add_trace(go.Scatter(x=df_active_t.index, y=df_active_t.iloc[:,selected_row],
                             mode='lines+markers',
                             name='Active',
...
def draw_singleCountry_Bar(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,selected_row=0,graph_line='Bar Chart'):
    ...
    if graph_line=='Bar Chart':
        ...
        fig.add_trace(go.Bar(x=df_active_t.index, y=df_active_t.iloc[:,selected_row],
                            name='Active',
                            marker_color='#f1f772'
                            ))
...
app.layout = html.Div(
    html.Div([
        ...
        html.Div(
            [
                html.H1(children='Corona Virus (Covid-19) Spread Interactive Dashboard',
                        style={
                            'textAlign': 'left',
                            'color': colors['text'],
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.P(f"{df_confirmed_total[-1]-(df_deaths_total[-1] + df_recovered_total[-1]):,d}",
                       style={
                    'textAlign': 'center',
                    'color': colors['active_text'],
                    'fontSize': 30,
                ...
                html.P('Past 24hrs increase:' + f"{(df_confirmed_total[-1]-(df_deaths_total[-1] + df_recovered_total[-1])) - (df_confirmed_total[-2]-(df_deaths_total[-2] + df_recovered_total[-2])):,d}"
                       + ' (' + str(round((((df_confirmed_total[-1]-(df_deaths_total[-1] + df_recovered_total[-1])) - (df_confirmed_total[-2]-(df_deaths_total[-2] + df_recovered_total[-2])))/(df_confirmed_total[-1]-(df_deaths_total[-1] + df_recovered_total[-1])))*100, 2)) + '%)',
                       style={
                    'textAlign': 'center',
                    'color': colors['active_text'],
                }
                ...
                html.Div(
                    [
                        dcc.RadioItems(
                            id='map-disp-type',
                            options=[{'label': i, 'value': i}
                                     for i in ['confirmed','active','deaths','recovered']],
                            value='confirmed',
                            labelStyle={'display': 'inline-block'},
                            style={
                                'fontSize': 20,
...
def map_selection(data, selected_rows,graph_line,map_disp_type):
    ...
    if len(selected_rows) == 0:
        fig1 = draw_singleCountry_Scatter(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,0)
        fig2 = draw_singleCountry_Bar(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,0,graph_line)
        return gen_map(aux,zoom,1.2833,103.8333,map_disp_type),fig1,fig2
    else:
        fig1 = draw_singleCountry_Scatter(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,selected_rows[0])
        fig2 = draw_singleCountry_Bar(df_confirmed_t,df_deaths_t,df_recovered_t,df_active_t,selected_rows[0],graph_line)
        zoom=3
        return gen_map(aux,zoom,temp_df['Lat'].iloc[0],temp_df['Long'].iloc[0],map_disp_type),fig1,fig2
